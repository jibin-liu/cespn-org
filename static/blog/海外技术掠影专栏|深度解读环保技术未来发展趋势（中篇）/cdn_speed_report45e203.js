...
if(t.mp_shop_info){
var P=t.mp_shop_info.pid||"",T=t.mp_shop_info.outer_id||"";
a=utils.extend({
pid:P,
outer_id:T,
pt:v,
...
var m=o.advertisement_num,u=o.advertisement_info;
window.adDatas.realNum=m;
var f=separateAdData(u);
u=f.oldAdInfos,m=u.length,(new AdFrame).handleAdWithFrame(f.newAdInfos),window.adDatas.num=m;
for(var g=0;m>g;++g){
var y,D=null,A=u[g];
A.exp_info=A.exp_info||{},A.is_cpm=A.is_cpm||0,A.biz_info=A.biz_info||{},A.app_info=A.app_info||{},
A.pos_type=A.pos_type||0,A.logo=A.logo||"",A.use_new_protocol=A.use_new_protocol||0;
var v=A.pt,P=A.pos_type,T=A.product_type;
if(2==A.use_new_protocol&&A.pos_type==AD_POS.POS_BOTTOM){
...
CrtManager.CRT_CONF[A.crt_size]||(A.use_new_protocol=A.product_type!=AD_TYPE.IOS_APP_PRODUCT_TYPE&&A.product_type!=AD_TYPE.ANDROID_APP_PRODUCT_TYPE||2!=A.material_type&&9!=A.material_type||A.dest_type!=AD_CONFIG.AD_DEST_TYPE.APPDETAIL_DEST_TYPE&&A.dest_type!=AD_CONFIG.AD_DEST_TYPE.APPINFO_PAGE_DEST_TYPE&&!AD_CONFIG.AD_DEST_TYPE.CANVAS_AD_DEST_TYPE?0:1,
console.info("[底部广告旧协议兼容] crt_size:",A.crt_size," 最终协议类型：",A.use_new_protocol),ajax({
url:"/mp/advertisement_report?action=extra_report&extra_data="+h+"&__biz="+biz,
timeout:2e3
}));
}
if(urlParser.getQuery("oldAd")&&(A.use_new_protocol=0),y=A.use_new_protocol,n[P]||(n[P]=0),
n[P]++,y)1==y?P===AD_POS.POS_MID?(c=!0,A=t(A),D=A):0===P?(A=i(A),(T===AD_TYPE.ANDROID_APP_PRODUCT_TYPE||T===AD_TYPE.IOS_APP_PRODUCT_TYPE)&&(D=A)):3===P&&(D=A):2==y&&(P===AD_POS.POS_MID?(c=!0,
A=t(A)):0==P&&(A=i(A)),D=A);else if(100===v||115===v){
for(var E=A.exp_info.exp_value||[],w=!1,O=0,b=0;b<E.length;++b){
var x=E[b]||{};
if(1==x.exp_type&&(O=x.comm_attention_num,w=O>0),2==x.exp_type){
w=!1,O=0;
break;
}
}
A.biz_info.show_comm_attention_num=w,A.biz_info.comm_attention_num=O,D={
usename:A.biz_info.user_name,
pt:v,
url:A.url,
traceid:A.traceid,
...
adid:A.aid,
is_appmsg:!0
};else if(103===v||104===v||2===v&&A.app_info){
var I=A.app_info.down_count||0,S=A.app_info.app_size||0,C=A.app_info.app_name||"",j=A.app_info.category,k=["万","百万","亿"];
if(I>=1e4){
I/=1e4;
for(var N=0;I>=10&&2>N;)I/=100,N++;
I=I.toFixed(1)+k[N]+"次";
}else I=I.toFixed(1)+"次";
S=utils.formSize(S),j=j?j[0]||"其他":"其他",C=utils.formName(C),A.app_info._down_count=I,
A.app_info._app_size=S,A.app_info._category=j,A.app_info.app_name=C,D={
appname:A.app_info.app_name,
app_rating:A.app_info.app_rating||0,
icon_url:A.app_info.icon_url,
app_id:A.app_info.app_id,
...
var Y=A.card_info.card_id||"",M=A.card_info.card_ext||"";
M=M.htmlDecode();
try{
M=JSON.parse(M),M.outer_str=A.card_info.card_outer_id||"",M=JSON.stringify(M);
}catch(R){
M="{}";
}
D={
...
var U=A.mp_shop_info.pid||"",z=A.mp_shop_info.outer_id||"";
D={
pid:U,
outer_id:z,
pt:v,
...
}else if(108===v||109===v||110===v||116===v||117===v)D={
pt:v,
ticket:A.ticket||"",
url:A.url,
traceid:A.traceid,
adid:A.aid,
is_appmsg:!0
},A.video_info&&(D.displayWidth=A.video_info.displayWidth,D.displayHeight=A.video_info.displayHeight,
D.thumbUrl=A.video_info.thumbUrl,D.videoUrl=A.video_info.videoUrl,D.rl=A.rl),A.dest_type==AD_CONFIG.AD_DEST_TYPE.WECHAT_APPLET_DEST_TYPE&&Wxopen_card.startConnect(A);else if(111===v||113===v||114===v||112===v||121===v||122===v){
var S=A.app_info.app_size||0,C=A.app_info.app_name||"";
S=utils.formSize(S),C=utils.formName(C),A.app_info.app_size=S,A.app_info.app_name=C,
D={
appname:A.app_info.app_name,
app_rating:A.app_info.app_rating||0,
app_id:A.app_info.app_id,
...
}else if(118===v)D=A,c=!0,Wxopen_card.startConnect(A);else if(119===v||120===v)D=A,
Wxopen_card.startConnect(A);else if(125===v)D=A,A.dest_type==AD_CONFIG.AD_DEST_TYPE.WECHAT_APPLET_DEST_TYPE&&Wxopen_card.startConnect(A);else if(140===v){
D=A;
try{
...
A.image_url=urlParser.addParam(q,"wxfrom","50",!0)),adDatas.ads[utils.getPosKeyDesc(P,n[P]-1)]={
a_info:A,
adData:D
},localStorage&&localStorage.setItem&&A.app_info&&A.app_info.url_scheme&&localStorage.setItem("__WXLS__a_url_schema_"+A.traceid,A.app_info.url_scheme),
A.has_installed=!1,A.app_info&&!function(e){
...
}(A),0===P&&9===A.material_type&&(T===AD_TYPE.MINI_GAME_PRODUCT_TYPE&&A.dest_type===AD_CONFIG.AD_DEST_TYPE.WECHAT_APPLET_DEST_TYPE&&A.game_info&&(A.biz_info.head_img=A.game_info.head_img,
A.biz_info.nick_name=A.game_info.nick_name),T!==AD_TYPE.IOS_APP_PRODUCT_TYPE&&T!==AD_TYPE.ANDROID_APP_PRODUCT_TYPE||!A.app_info||(A.biz_info.head_img=A.app_info.icon_url,
A.biz_info.nick_name=A.app_info.app_name));
}
var B=function _a(){
...
for(var H in W)if(0===H.indexOf("pos_")){
var D=W[H],A=!!D&&D.a_info,T=A.product_type;
if(D&&A){
if(insertAutoAdElement(A),2!==A.use_new_protocol){
if(0==A.pos_type){
if(A.new_appmsg=window.new_appmsg,A.longAppBtnText=T===AD_TYPE.IOS_APP_PRODUCT_TYPE?"查看应用":"下载应用",
A.shortAppBtnText=T===AD_TYPE.IOS_APP_PRODUCT_TYPE?"查看":"下载",js_bottom_ad_area.innerHTML=TMPL.tmpl(a_tpl,A),
111==A.pt||112==A.pt||113==A.pt||114==A.pt){
var V=document.getElementsByClassName("js_download_app_card")[0],L=V.offsetWidth,G=Math.floor(L/2.875);
G>0&&(V.style.height=G+"px");
}
}else if(3==A.pos_type){
var V=document.createElement("div");
V.appendChild(document.createTextNode(A.image_url)),A.image_url=V.innerHTML.replace(/&amp;/g,"&"),
A.new_appmsg=window.new_appmsg,js_sponsor_ad_area.innerHTML=TMPL.tmpl(sponsor_a_tpl,A),
js_sponsor_ad_area.style.display="block";
var K=js_sponsor_ad_area.clientWidth;
108!=A.pt&&109!=A.pt&&110!=A.pt||2==A.use_new_protocol?116==A.pt||117==A.pt:document.getElementById("js_main_img").style.height=K/1.77+"px",
DomEvent.on(window,"scroll",B),B(0);
}else if(A.pos_type===AD_POS.POS_MID&&utils.checkShowCpc(el_gdt_areas[H],globalAdDebug,contentWrp,A)){
A=_parseExpCpc(A);
var J=!1;
if(T===AD_TYPE.ANDROID_APP_PRODUCT_TYPE||T===AD_TYPE.IOS_APP_PRODUCT_TYPE)js_cpc_area.innerHTML=TMPL.tmpl(cpc_a_tpl,A),
J=!0;else{
var X=require("a/tpl/cpc_tpl.html.js"),Q=A.exp_obj.sale_text;
(T===AD_TYPE.ADD_CONTACT_PRODUCT_TYPE||T===AD_TYPE.ANDROID_APP_PRODUCT_TYPE||T===AD_TYPE.IOS_APP_PRODUCT_TYPE||T===AD_TYPE.MINI_GAME_PRODUCT_TYPE)&&(Q=A.avatarTitle),
js_cpc_area.innerHTML=TMPL.tmpl(X,{
tag_pos:A.exp_obj.tag_pos,
type:A.tag_pos,
ticket:A.ticket,
url:A.url,
rl:A.rl,
...
pos_type:A.pos_type,
banner:A.image_url,
price:A.exp_obj.price,
title:Q,
is_wx_app:A.is_wx_app,
btn_text:A.exp_obj.btn_text,
avatar:A.avatar,
isDownload:J
});
}
}
mmversion.isIOS&&A.app_info&&A.app_info.url_scheme&&T===AD_TYPE.IOS_APP_PRODUCT_TYPE&&(document.getElementById("js_promotion_tag")&&(document.getElementById("js_promotion_tag").innerHTML="查看应用"),
...
document.getElementById("js_ad_btn_"+A.pos_type)&&(document.getElementById("js_ad_btn_"+A.pos_type).innerHTML="查看应用"));
}else{
var Z,$={},J=!1,ea={};
if(A.button_action)try{
"string"==typeof A.button_action&&(A.button_action=JSON.parse(A.button_action.html())),
A.button_action.button_text&&""!=A.button_action.button_text||(A.button_action.button_text="Explore");
}catch(R){
A.button_action={
button_text:"decode error"
};
}else A.button_action={
button_text:"something wrong"
};
Z=A.crt_size,(T===AD_TYPE.ANDROID_APP_PRODUCT_TYPE||T===AD_TYPE.IOS_APP_PRODUCT_TYPE)&&(J=!0);
var Q="",aa="";
if(processAdAvatar(A),A.dest_type==AD_CONFIG.AD_DEST_TYPE.WECHAT_APPLET_DEST_TYPE&&Wxopen_card.startConnect(A),
A.pos_type===AD_POS.POS_MID)utils.checkShowCpc(el_gdt_areas[H],globalAdDebug,contentWrp,A)&&(A=_parseExpCpc(A),
(A.avatarTitle||A.exp_obj.sale_text)&&A.avatar&&(Q=A.avatarTitle||A.exp_obj.sale_text,
aa=A.avatar),$={
tag_pos:A.exp_obj.tag_pos,
type:A.tag_pos,
ticket:A.ticket,
...
}),ad_render_object[H]=new CrtManager.createCrtObject(Z,$,el_gdt_areas[H],ea),gdt_as[H]=el_gdt_areas[H].getElementsByClassName("js_ad_main_area"));else if(A.pos_type==AD_POS.POS_SPONSOR){
var V=document.createElement("div");
V.appendChild(document.createTextNode(A.image_url)),A.image_url=V.innerHTML.replace(/&amp;/g,"&"),
A.new_appmsg=window.new_appmsg;
var D={
pt:A.pt,
ticket:A.ticket||"",
url:A.url,
...
if(A.video_info&&(D.displayWidth=A.video_info.displayWidth,D.displayHeight=A.video_info.displayHeight,
D.thumbUrl=A.video_info.thumbUrl,D.videoUrl=A.video_info.videoUrl,D.rl=A.rl),$={
type:A.tag_pos,
ticket:A.ticket,
url:A.url,
rl:A.rl,
...
pos_type:A.pos_type,
banner:A.image_url,
title:A.biz_info.nick_name,
is_wx_app:A.button_action.jump_type==AD_CONFIG.AD_DEST_TYPE.WECHAT_APPLET_DEST_TYPE,
is_ios:mmversion.isIOS,
isDownload:J,
btn_text:A.button_action.button_text,
avatar:A.biz_info.head_img,
isApp:!1
},ad_render_object[H]=new CrtManager.createCrtObject(Z,$,js_sponsor_ad_area,ea),
js_sponsor_ad_area.style.display="block",gdt_as["pos_"+A.pos_type]=js_sponsor_ad_area.getElementsByClassName("js_ad_main_area"),
!ad_render_object[H].getCrtData().has_banner){
var ta="&tid="+A.traceid+"&uin="+uin+"&key="+l+"&ticket="+(A.ticket||"")+"&__biz="+biz+"&source="+source+"&scene="+scene+"&appuin="+biz+"&aid="+A.adid+"&ad_engine=0&pos_type="+A.pos_type+"&exp_id="+A.exp_info.exp_id+"&exp_value="+A.exp_info.exp_value+"&r="+Math.random();
A.report_param=ta;
}
...
}else if(A.pos_type==AD_POS.POS_BOTTOM){
var ea={};
if(A.video_info&&(D.displayWidth=A.video_info.displayWidth,D.displayHeight=A.video_info.displayHeight,
D.thumbUrl=A.video_info.thumbUrl,D.videoUrl=A.video_info.videoUrl,D.rl=A.rl),J&&(ea.customUpdataFunc=function(e,a){
var t=e.querySelector(".js_download_percent"),i=e.querySelector(".js_download_outside"),n=e.querySelector(".js_download_inner");
t&&(t.style.width=a.percent+"%"),i.textContent=a.btn_text,n.textContent=a.btn_text;
...
i.indexOf("get_install_state:yes")>-1&&A.app_info.url_scheme&&(n.textContent=354==parseInt(a.crt_size)||117==parseInt(a.crt_size)||355==parseInt(a.crt_size)||568==parseInt(a.crt_size)?"Enter":"Open App");
});
}),A.avatarTitle&&A.avatar&&(Q=A.avatarTitle||A.exp_obj.sale_text,aa=A.avatar),$=utils.extend({
banner:A.image_url,
is_wx_app:A.button_action.jump_type==AD_CONFIG.AD_DEST_TYPE.WECHAT_APPLET_DEST_TYPE,
btn_text:A.button_action.button_text,
avatar:A.avatar,
...
isDownload:J,
title:Q
},A),ad_render_object["pos_"+A.pos_type]=new CrtManager.createCrtObject(Z,$,js_bottom_ad_area,ea),
!ad_render_object["pos_"+A.pos_type].getCrtData().has_banner){
var ta="&tid="+A.traceid+"&uin="+uin+"&key="+l+"&ticket="+(A.ticket||"")+"&__biz="+biz+"&source="+source+"&scene="+scene+"&appuin="+biz+"&aid="+A.adid+"&ad_engine=0&pos_type="+pos_type+"&exp_id="+A.exp_info.exp_id+"&exp_value="+A.exp_info.exp_value+"&r="+Math.random();
A.report_param=ta;
var na=ad_render_object["pos_"+A.pos_type].getWrapperElm().getElementsByClassName("js_video_container_new_protocol");
na[0]&&(na=na[0],A.videoContainer=na,videoAdMap[A.aid]=new VideoAd(A));
}
gdt_as["pos_"+A.pos_type]=js_bottom_ad_area.getElementsByClassName("js_ad_main_area"),
setBottomSize(A);
}
}
utils.lazyLoadAdImg({
...
has_click[n]=!0;
{
var A=document.getElementById("loading_"+n);
g.product_type;
}
...
if(has_click[n]=!1,A&&(A.style.display="none"),g.app_info){
...
var i,r,l=a.type,c=a.url,u=a.rl,g=a.apurl,y=a.ticket,D=a.group_id,A=a.pt,v=p.use_new_protocol;
i=m[0].clientWidth,r=m[0].clientHeight;
var P=getClickEventPageOffset(e),T=offset.getOffset(m[0]),h=P.x-T.offsetLeft,E=P.y-T.offsetTop;
ad_click(l,c,u,g,o,y,D,n,A,d,s,_,h,E,i,r,p,f,v),log("[Ad] ad_click: type="+l+", url="+c+", rl="+u+", apurl="+g+", traceid="+o+", ticket="+y+", group_id="+D+", aid="+n+", pt="+A+", pos_type="+d+", ad_engine="+s);
}
...
var A=require("a/profile.js");
_.adid=window.adid||_.adid||_.aid,new A({
btnProfile:u[0],
adData:_,
pos_type:d,
report_param:f,
...
_.adid=window.adid||_.adid||_.aid;
var A="&tid="+_.traceid+"&uin="+uin+"&key="+key+"&ticket="+(_.ticket||"")+"&__biz="+biz+"&source="+source+"&scene="+scene+"&appuin="+biz+"&aid="+_.adid+"&ad_engine="+s+"&pos_type="+d+"&r="+Math.random();
}
var v,P,T,h;
...
return ad_click(i,n,o,l,c,m,f,g,y,d,s,_,O,b,T,h,p,A,D),log("[Ad] ad_click: type="+i+", url="+n+", rl="+o+", apurl="+l+", traceid="+c+", ticket="+m+", group_id="+f+", aid="+g+", pt="+y+", pos_type="+d+", ad_engine="+s),
hideComplainBtns(),!1;
});
...
var D,A,v,P;
return D=position.getX(t,"js_ad_link")+e.offsetX,A=position.getY(t,"js_ad_link")+e.offsetY,
...
ad_click(r,l,c,m,o,u,f,n,g,d,s,a,D,A,v,P,p,_,y),log("[Ad] ad_click: type="+r+", url="+l+", rl="+c+", apurl="+m+", traceid="+o+", ticket="+u+", group_id="+f+", aid="+n+", pt="+g+", pos_type="+d+", ad_engine="+s),
!1;
}
...
var g=require("a/app_card.js"),y=15,D=_.pkgname&&_.pkgname.replace(/\./g,"_"),O=document.getElementById("js_ad_btn_"+d);
new g({
btn:O,
adData:_,
report_param:f,
...
if("102"==_.pt){
var b=require("a/android.js"),y=15,D=_.pkgname&&_.pkgname.replace(/\./g,"_");
return void new b({
...
var A="",v="";
d&&(A=d.pkgname&&d.pkgname.replace(/\./g,"_"),v=d.channel_id||""),y={
source:4,
tid:a,
traceid:a,
mid:i,
...
classList.addClass(a,"ad_after_video_container"),this.aInfoMap[e.aid].iframeEle=a;
...
void t.initAdBeforeVideo(e)):e.pos_type===AD_POS.POS_BOTTOM?void t.initBottomAd(e):e.pos_type===AD_POS.POS_AD_AFTER_VIDEO?(utils.report115849(33),
void t.initAdAfterVideo(e)):void 0;
}),this.bindAdEvent();
}
},initAdData(),{
